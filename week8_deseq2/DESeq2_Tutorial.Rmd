...
rna_clinical <- read.csv("C:/Users/kenne/OneDrive/Desktop/QBIO490/qbio_490_kenneth/analysis_data/brca_rna_clinical_data.csv", header = TRUE)
...
load("/PATH/TO/workspace_file.RData")
>>>>>>> 39396c090316fdffce6b03721f577b9adf55411b
```
...
Levels: american indian or alaska native, asian, black or african american, not reported, white

```{r}
# write code here
factor(rna_clinical$paper_BRCA_Subtype_PAM50)
...
```{r}
na_mask <- ifelse(is.na(rna_clinical$age_category) | is.na(rna_clinical$ajcc_pathologic_stage) | is.na(rna_clinical$paper_BRCA_Subtype_PAM50), FALSE, TRUE) # you can use multiple NA masks if you have multiple columns with NA values (or you can do it all in one!)
...
row_sums <- rowSums(rna_counts[,2:1083])
...
#rna_clinical = rna_clinical[, 3:88]
#rownames(rna_clinical) = rna_clinical$barcode
#colnames(rna_counts) <- gsub(".", "-", colnames(rna_counts), fixed = TRUE)
#rownames(rna_counts) = rna_counts$X
#rna_counts = rna_counts[, 2:1083]
```
...
dds = DESeqDataSetFromMatrix(countData = rna_counts,
                             colData = rna_clinical, design = ~age_category + ajcc_pathologic_stage +paper_BRCA_Subtype_PAM50)
...
load("C:/Users/kenne/OneDrive/Desktop/QBIO490/qbio_490_kenneth/deseq_output.RData")          

=======
results = results(dds_obj, format = "DataFrame", contrast = c("age_category", "FACTOR 1", "FACTOR 2")) # this is case sensitive so be careful to match it with your age_category factors closely!
>>>>>>> 39396c090316fdffce6b03721f577b9adf55411b
```
...
```{r}
# write code here
mask = ifelse(rna_genes$gene_id %in% rownames(results), TRUE, FALSE)
results <- data.frame(rna_genes$gene_name[mask], rownames(results), results$log2FoldChange, results$pvalue, results$padj, -log10(results$padj))
...
```{r}
sig_results <- ifelse(results$padj < 0.05, TRUE, FALSE)
```
...
```{r}
sig_results_df = results[sig_results, ]
ordered_indices = order(sig_results_df$log2FoldChange)
up_reg_results <- sig_results_df[ordered_indices, ]
```
...
```{r}
# write code here
ordered_indices_decreasing = order(sig_results_df$log2FoldChange, decreasing = TRUE)
down_reg_results = sig_results_df[ordered_indices_decreasing, ]
...
```{r}
# write code here
row.names(up_reg_results) = 1:16931
row.names(down_reg_results) = 1:17101
```
...
Gene ensembl id: ENSG00000244155.1
Log2FoldChange: 1.001071
p-adj value: 3.351962e-02
...
The gene Cytochrome P450 Family 4 Subfamily F Member 34 (CYP4F34P) acts as a pseudogene, meaning that it is not actually a functional gene.

Gene ensembl id: ENSG00000266977.2
Log2FoldChange: -1.000077
p-adj value: 0.0098254435
More highly expressed in young or old patients: old
Google the gene. What is its full name, and what appears to be its function? Genecards generally has a pretty good summary:
The gene NONHSAG043414.2 (AC006504.2) does not appear to have a known function.
...
head(up_reg_results)
head(down_reg_results)
write.csv(results, "C:/Users/kenne/OneDrive/Desktop/QBIO490/qbio_490_kenneth/analysis_data/brca_DESeq2_results.csv")
write.csv(up_reg_results, "C:/Users/kenne/OneDrive/Desktop/QBIO490/qbio_490_kenneth/analysis_data/brca_DESeq2_up_reg_results.csv")
write.csv(down_reg_results, "C:/Users/kenne/OneDrive/Desktop/QBIO490/qbio_490_kenneth/analysis_data/brca_DESeq2_down_reg_results.csv")
```
...
top-right genes: up-regulated in young patients, significant
bottom-right genes: up-regulated in young patients, not significant
top-left genes: up-regulated in old patients, significant
bottom-left genes: up-regulated in old patients, not significant
top-middle genes: no difference in regulation, significant
bottom-middle genes: no difference in regulation, not significant
...
```{r}
# create plot here
jpeg("C:/Users/kenne/OneDrive/Desktop/QBIO490/qbio_490_kenneth/week8_deseq2/volcano_plot.jpeg")
par(mar=c(4,4,4,4))
EnhancedVolcano(results,
    lab = rownames(results),
    x = 'log2FoldChange',
