...
```{r setup}
    knitr::opts_knit$set(root.dir = normalizePath("C:/Users/kenne/OneDrive/Desktop/QBIO490/qbio_490_kenneth"))
```
...
# write code here
sum(na_boolean_mask)
#This represents the number of values in the station column of the attenu data frame that were "NA".
```
...
nrow(attenu)
nrow(attenu_cleaned)
#These values make sense since the attenu_cleaned data frame is the attenu data frame with rows containing "NA" for station removed, so attenu_cleaned has less rows than attenu.
```
...
```{r}
# write code here
boxplot(iris$Sepal.Length ~ iris$Species, xlab = "Species", ylab = "Sepal Length", main = "Sepal Lengths for Flower Species")
```
...
```{r}
# write code here
hist(iris$Petal.Length, main = "Frequency of Petal Lengths", xlab = "Petal Lengths")
```
...
colors = ifelse(iris$Sepal.Width >= mean(iris$Sepal.Width) & iris$Sepal.Length >= mean(iris$Sepal.Length), "red", 
  ifelse(iris$Sepal.Width >= mean(iris$Sepal.Width) & iris$Sepal.Length <= mean(iris$Sepal.Length), "orange",
  ifelse(iris$Sepal.Width <= mean(iris$Sepal.Width) & iris$Sepal.Length <= mean(iris$Sepal.Length), "yellow", "green"
  )))
colors = factor(colors)
print(cor.test(iris$Sepal.Width, iris$Sepal.Length))
plot(x = iris$Sepal.Width, y = iris$Sepal.Length, col = colors)
abline(v = mean(iris$Sepal.Width))
...
```{r}
# write code 
colors2 = ifelse(iris$Species == "setosa", "red", 
  ifelse(iris$Species == "versicolor", "orange", "yellow"))
colors2 = factor(colors2)
plot(iris[, 1:4], col = colors2)
```
...
```{r}
# write code here
write.csv(attenu_cleaned, "C:/Users/kenne/OneDrive/Desktop/QBIO490/qbio_490_kenneth/attenu_cleaned.csv")
```
...
```{r}
# write code here
df <- read.csv("C:/Users/kenne/OneDrive/Desktop/QBIO490/qbio_490_kenneth/attenu_cleaned.csv", header = TRUE)
```
...
```{r}
# write code here
jpeg("C:/Users/kenne/OneDrive/Desktop/QBIO490/qbio_490_kenneth/petalLengths.jpeg")
hist(iris$Petal.Length, main = "Frequency of Petal Lengths", xlab = "Petal Lengths")
dev.off()
```
